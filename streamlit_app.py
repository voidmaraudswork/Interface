...
<head>
    ...
    <style>
        ...
        .modal { 
            ...
            background: rgba(0,0,0,0.96); z-index: 8000; display: none; 
        ...
        /* OVERRIDE TERMINAL */
        #override-input {
            width: 90%; background: transparent; border: none; border-bottom: 2px solid #ff0055;
            color: #ff0055; font-family: 'Orbitron'; text-align: center; font-size: 0.8rem;
            margin: 20px 0; outline: none; letter-spacing: 2px;
        }
        .override-h { color: #ff0055 !important; font-size: 0.9rem; text-shadow: 0 0 10px #ff0055; }
        ...
        .confirm-btn { width: 100%; padding: 15px; border: 1px solid #00f2ff; background: transparent; color: #00f2ff; font-family: 'Orbitron'; cursor: pointer; transition: 0.3s; }
        .confirm-btn.ready { background: rgba(0, 242, 255, 0.1); }
...
<body>
    ...
    <div id="override-modal" class="modal">
        <div class="modal-box" style="border-color:#ff0055; box-shadow: 0 0 40px rgba(255,0,85,0.4);">
            <h2 class="override-h">UNAUTHORIZED ACCESS</h2>
            <p style="font-family:'Rajdhani'; color:#777; font-size:0.7rem; margin-top:8px;">This module is locked by Protocol V5. Enter override code to bypass.</p>
            <input type="text" id="override-input" placeholder="ENTER ACCESS CODE..." autocomplete="off">
    ...
    <div class="grid">
        <div class="btn-container" id="btn-1" onclick="handleInteraction('btn-1', 'AI CODE FLATTENER', 'https://aicodeflat.streamlit.app/')">
            <div class="btn-inner"><b>AI CODE FLATTENER</b></div>
        </div>
    ...
    <script>
        ...
        let storage = JSON.parse(localStorage.getItem('void_protocol_final')) || { active: [], locked: [], expiry: null };
        if(storage.expiry && Date.now() > storage.expiry) { storage = { active: [], locked: [], expiry: null }; localStorage.removeItem('void_protocol_final'); }
        ...
        function checkOverride() {
            ...
            if(input === 'ifollowedvoidmarauds') {
                storage.active.push(currentAttemptId);
                storage.locked = storage.locked.filter(id => id !== currentAttemptId);
                localStorage.setItem('void_protocol_final', JSON.stringify(storage));
        ...
        function toggleSelect(el, id) {
            ...
            if(selectedIds.length === 2) btn.style.opacity = '1', btn.style.pointerEvents = 'auto'; else btn.style.opacity = '0.3', btn.style.pointerEvents = 'none';
        }
...
""", height=1000)import streamlit as st
import streamlit.components.v1 as components
